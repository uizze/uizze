 ...
 * an ignore in `.uizze/config.json`.
 *
 ...
 * line, so the same marker works across every comment style uizze scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   uizze-disable <rule>[, <rule>...] [-- reason]   whole file
 *   uizze-disable-line <rule>...      [-- reason]   the same line
 *   uizze-disable-next-line <rule>... [-- reason]   the following line
 *   uizze-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 *   <!-- uizze-disable overused-font -- exported brand doc, font is first-party -->
 *   .brand { font-family: Inter; } /* uizze-disable-line overused-font *\/
 *   // uizze-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /uizze-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `Uizze-Disable`).
  if (!/uizze-disable/i.test(text)) return result;
