...
function findLeftoverUizzeMarker(content) {
  const commentMarker = content.match(/^\s*(?:<!--|\{\/\*)\s*uizze-carbonize-(?:start|end)\b|^\s*(?:<!--|\{\/\*)\s*uizze-variants-(?:start|end)\b/m);
...
function runCodex(prompt, { cwd, env, resultPath, logPath, timeoutMs = DEFAULT_TIMEOUT_MS }) {
  const args = [
    ...
    '-c', `model_reasoning_effort="${env.UIZZE_LIVE_COPY_AGENT_EFFORT || 'low'}"`,
  ];
...
export function describeNoProviderError({
  ...
  if (chatAvailable()) {
    ...
    lines.push('  • Chat: no Uizze live session is currently polling on this server. Start Uizze live in your chat to route Apply through the chat agent.');
  }
