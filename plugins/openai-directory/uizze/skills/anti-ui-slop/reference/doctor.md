Report and repair drift between this project's Uizze artifacts and what the installed version reads: PRODUCT.md, DESIGN.md and its `.uizze/design.json` sidecar, `.uizze/config.json`, persisted surface briefs, and the design hook.
...
- **Tool version.** The installed skill is older than the published one. `context.mjs` reports that at boot as `UPDATE_AVAILABLE` and `reinstall the current Uizze skill archive` fixes it. Not this command's job.
- **Schema drift.** An artifact was written by an older Uizze: fields nothing reads, fields now expected, files in retired locations. Mechanical, and this command repairs most of it.
...
- **`auto`** carries no decision. Run `node scripts/doctor.mjs --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
...
`context.mjs` reports the cheap subset of these findings at session start, throttled to once a week per project. Set `"stalenessCheck": false` in `.uizze/config.json` to silence that, or `UIZZE_NO_STALENESS_CHECK=1` for one session. This command still works with the check disabled, and that is the combination to suggest for a user who wants the report only when they ask for it.
