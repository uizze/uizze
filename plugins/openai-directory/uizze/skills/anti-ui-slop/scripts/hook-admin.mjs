...
function stripUizzeHookEntry(entry) {
  ...
  if (valueHasUizzeHookMarker(entry.command) || valueHasUizzeHookMarker(entry.args)
    || valueHasUizzeHookMarker(entry.bash) || valueHasUizzeHookMarker(entry.powershell)) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks
    .map(stripUizzeHookEntry)
    .filter(Boolean);
...
function addIgnoreRule(cwd, args) {
  ...
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error(`overused-font is value-specific by default. Use ${UIZZE_COMMAND} hooks ignore-value overused-font <font> for a confirmed font, or ${UIZZE_COMMAND} hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.`);
  }
...
function addIgnoreValue(cwd, args) {
  ...
  if (parsed.value === '*' && parsed.files.length === 0) {
    ...
    throw new Error(`Wildcard value ignores must be scoped with --file <glob>, e.g. ${UIZZE_COMMAND} hooks ignore-value design-system-font-size "*" --file "src/widget.js". To suppress the rule project-wide use ${projectWide}.`);
  }
