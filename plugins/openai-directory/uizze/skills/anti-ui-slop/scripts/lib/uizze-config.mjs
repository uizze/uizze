/**
 * CLI-side reader/writer for the unified `.uizze` config.
 *
...
export function getConfigPath(root) {
  return join(root, '.uizze', 'config.json');
}

export function getLocalConfigPath(root) {
  return join(root, '.uizze', 'config.local.json');
}
...
function applyDetectionConfigSource(config, raw) {
  ...
  // so config round-trips (e.g. `uizze hooks ignore-value`) preserve it.
...
const EXCLUDE_OPEN = '# uizze-config-ignore-start';
const EXCLUDE_CLOSE = '# uizze-config-ignore-end';
const EXCLUDE_PATTERNS = ['.uizze/config.local.json'];
