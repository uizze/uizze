...
// boundaries; `.uizze` is our own project marker.
const PROJECT_ROOT_MARKERS = ['.git', 'package.json', '.uizze'];
...
function resolveDesignSidecarPath(cwd = process.cwd(), contextDir = cwd) {
  const candidates = [
    path.join(cwd, '.uizze', 'design.json'),
...
//   - A directory carrying a project marker (.git / package.json / .uizze)
