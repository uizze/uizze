...
// ─── Scoped ignores: data-uizze-ignore ─────────────────────────────────
//
// An element-scoped waiver that travels with the markup: any element carrying
// `data-uizze-ignore="rule-a rule-b"` (or `*`, or an empty value, for
...
// `uizze-disable` comment directives, which the browser cannot apply (a
// live DOM has no line numbers), and the generalization of the one-off
// `data-uizze-allow-kickers` opt-out.
//
...
function scopedIgnoreActive(el, ruleId) {
  ...
  while (cur && cur.nodeType === 1) {
    const attr = typeof cur.getAttribute === 'function' ? cur.getAttribute('data-uizze-ignore') : null;
    if (attr != null) {
...
if (IS_BROWSER) {
  ...
  function addBrowserFindings(groupMap, el, findings) {
    ...
    // Element-scoped waivers: a data-uizze-ignore ancestor suppresses
  ...
  function browserDesignSystemConfig() {
    const raw = window.__UIZZE_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    // ground truth in the browser), and a match under a data-uizze-ignore
  ...
  function visualContrastMode(options = {}) {
    const explicit = typeof options.visualContrast === 'boolean'
      ...
      : typeof window.__UIZZE_CONFIG__?.visualContrast === 'boolean'
        ? window.__UIZZE_CONFIG__.visualContrast
        : null;
  ...
  window.uizzeMeasureHiddenText = measureHiddenTextDOM;
