...
(function () {
  ...
  function stripManualEditRuntimeState(root) {
    ...
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-uizze-editable')
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: uizze-configure-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function mountedParameterCount() {
    ...
    const wrapper = document.querySelector('[data-uizze-variants="' + currentSessionId + '"]');
    if (!wrapper) return 0;
    return [...wrapper.querySelectorAll('[data-uizze-variant]:not([data-uizze-variant="original"])')]
      ...
      .then(html => {
        ...
        if (arrivedVariants <= 0) {
          if (state === 'GENERATING') {
            ...
            if (attempt < COMPLETED_SOURCE_FALLBACK_RETRIES) {
              console.log('[uizze] Generation is done but source shows no variants yet; retrying read ('
  ...
  function updateVariantStateStylesheet(sessionId, num) {
    ...
    const hideOthers = variantStateSelector(sessionId)
      + ':not([data-uizze-variant="' + num + '"]) { ' + VARIANT_HIDE_DECL + ' }';
  ...
  function showOriginalDuringDiscard(sessionId) {
    ...
    const wrapper = '[data-uizze-variants="' + sessionId + '"]';
    styleEl.textContent = wrapper + ' > [data-uizze-variant]:not([data-uizze-variant="original"]) { display:none !important; }\n'
      + wrapper + ' > [data-uizze-variant="original"] { display:block !important; }';
  }
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        ...
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-uizze-variants="' + sessionId + '"]') || n.querySelector?.('[data-uizze-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-uizze-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast(`No PRODUCT.md found. Variants will be brand-agnostic. Run ${UIZZE_COMMAND} init to generate one.`, 7000);
...
void main() {
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-uizze-variants],[data-uizze-variant],[data-uizze-carbonize]'));
  }
  ...
  function resumeSession() {
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.uizzePreview)) {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.uizzeVariantCount)
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('uizze-steer-debug') === '1'; } catch { return false; }
  }
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        ...
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: uizze-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: uizze-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', agentStatusMessage
      ? 'Uizze live mode - ' + (agentHasWorkInFlight() ? 'agent is working' : 'agent not polling')
      : 'Uizze live mode');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: uizze-agent-dot 1.4s ease-in-out infinite; }' +
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return 'DESIGN.md found, no structured tokens to display. Run ' + UIZZE_COMMAND + ' document to generate the .uizze/design.json sidecar.';
    }
  ...
  function init() {
    ...
    if (!resumeSession()) {
      ...
      console.log('[uizze] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
